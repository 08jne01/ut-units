...
concept compatible_qty =
    ...
    same_dimensions<typename T1::dimensions, typename T2::dimensions>::value    &&
    std::same_as<T1,T2>;

template<typename T1, typename T2>
concept convertible_qty = 
...
typename TyDimensions
>
UT_UNITS_CRITICAL_INLINE constexpr T qty<T,TyDimensions>::in( const qty_offset<T,TyDimensions>& other) const
{
    return (value / other.value) - other.offset;
}

template<std::floating_point T, typename T1, typename T2>
struct qty_multiply
{
    using value = qty<
        T,
...
requires(sqrtable<dimensions>::value)
struct qty_sqrt
{
    using value = qty<
        T,
...
template<int N, std::floating_point T, typename dimensions>
struct qty_pow
{
...
template<
    ...
    typename TyRight
>
[[nodiscard]] UT_UNITS_CRITICAL_INLINE constexpr qty_multiply<T, TyLeft, TyRight>::value
operator*(qty<T,TyLeft> left, qty<T,TyRight> right)
{
    typename qty_multiply<T, TyLeft,TyRight>::value value;
...
template<
    ...
    typename TyRight
>
[[nodiscard]] UT_UNITS_CRITICAL_INLINE constexpr qty_divide<T, TyLeft, TyRight>::value
operator/( qty<T,TyLeft> left, qty<T,TyRight> right)
{
    typename qty_divide<T, TyLeft,TyRight>::value value;
    value.value = left.value / right.value;
    return value;
}
...
[[nodiscard]] UT_UNITS_CRITICAL_INLINE constexpr qty<T,TyDimension> operator-(qty<T,TyDimension> value)
{
    value.value = -value.value;
    return value;
}

template<std::floating_point T, typename TyDimension, std::convertible_to<T> Ty>
[[nodiscard]] UT_UNITS_CRITICAL_INLINE constexpr qty<T,TyDimension> operator*(Ty left, qty<T,TyDimension> right)
{
    right.value *= left;
    return right;
}
...
template<std::floating_point T, typename TyDimension>
[[nodiscard]] UT_UNITS_CRITICAL_INLINE constexpr qty<T,TyDimension> operator*(qty<T,TyDimension> left, T right)
{
    left.value *= right;
    return left;
}
...
template<std::floating_point T, typename TyDimension>
[[nodiscard]] UT_UNITS_CRITICAL_INLINE constexpr qty_divide<T, qty_dimensions<>, TyDimension>::value operator/(T left, qty<T,TyDimension> right)
{
    typename qty_divide<T, qty_dimensions<>, TyDimension>::value result;
...
template<std::floating_point T, typename TyDimension>
[[nodiscard]] UT_UNITS_CRITICAL_INLINE constexpr qty<T,TyDimension> operator*(T left, qty_offset<T,TyDimension> right)
{
    qty<T,TyDimension> result;
...
namespace ut
{
    ...
    template<std::floating_point T, typename dimensions>
    [[nodiscard]] UT_UNITS_CRITICAL_INLINE qty_sqrt<T,dimensions>::value sqrt( qty<T,dimensions> value )
    {
        typename qty_sqrt<T,dimensions>::value result;
        result.value = std::sqrt(value.value);
        return result;
    }

    template<int N, std::floating_point T, typename dimensions>
    [[nodiscard]] UT_UNITS_CRITICAL_INLINE constexpr qty_pow<N,T,dimensions>::value pow( qty<T,dimensions> value )
    {
        typename qty_pow<N,T,dimensions>::value result;
        result.value = 1.0;
